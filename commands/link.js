...
module.exports = {
  ...
  execute: async (message, args, client) => {
    ...
    try {
      ...
      if (args.length == 0) return message.author.send("Enter a valorant username to link");
      ...
      if (puuid === null) return message.author.send("Not a valid Valorant Username, please try again");
    ...
    return res === true ? message.author.send("Successfully Linked") : message.author.send(`Failed to link, try running ${prefix}unlink and try again.`) ;
      ...
      console.log({msg: "Error running link command.", err: err})
